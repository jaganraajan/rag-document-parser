    ...
    for filename in os.listdir(directory):
        if filename.endswith('.pdf'):
            ...
            try:   
                ...
                status = "error"
                ingestion_errors_total.add(1, {"error.type": e.__class__.__name__})
                raise
            finally:
                duration = time.perf_counter() - start
                document_ingest_seconds.record(duration, {"status": status})
                if status == "success":
                    documents_ingested_total.add(1, {"status": "success"})
